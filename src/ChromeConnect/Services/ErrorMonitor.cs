...
namespace ChromeConnect.Services
{
    ...
    public class ErrorMonitor
    {
        ...
        {
            // For ChromeConnect exceptions, use the most specific type name
            string typeName = exception.GetType().Name;
            
            // Add error code if it's a ChromeConnect exception with an error code
            if (exception is ChromeConnectException ccException && 
                !string.IsNullOrEmpty(ccException.ErrorCode))
            {
                return $"{typeName}:{ccException.ErrorCode}";
            }
